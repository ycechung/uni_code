...
int topological_sort( DiGraph DG )
{
   ...
   else
   {
       ...
       while (isEmpty(Q) != 1) // while Q not empty
       {
          ...
          nodeEdgeType * pNode = DG -> L[x].head;
          while ( pNode ) // while an edge exists
          {
             D[pNode -> data]--; // decrement in degree of vertex pNode -> data
             if ( !D[pNode -> data] ) // insert in Q
             {
                  tempResult = insertQ( pNode -> data, Q );
             }      
             pNode = pNode -> next;
          }
       }        
       ...
       {  
          /* print the list in topological order */
